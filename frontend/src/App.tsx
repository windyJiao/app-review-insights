    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold flex items-center gap-3">📊 App Store Review Analyzer</h1>
              <p className="mt-1 text-sm text-gray-500">AI-powered review analysis → findings → PRD → test cases</p>
            </div>
            <div className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${backendOnline === null ? 'bg-yellow-400' : backendOnline ? 'bg-green-400' : 'bg-red-400'}`} />
              <span className="text-xs text-gray-400">{backendOnline === null ? 'Checking...' : backendOnline ? 'Connected' : 'Offline'}</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="flex space-x-4 mb-6">
          {(['analyze', 'import'] as const).map((m) => (
            <button key={m} onClick={() => { setMode(m); resetAll(); }}
              ...
              {m === 'analyze' ? '🔗 App Store' : '📁 Import Data'}
            </button>
          ))}
        </div>
        ...
        {error && (
          <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-lg">
            <h3 className="font-medium text-red-800">⚠️ Error</h3>
        ...
        {events.length > 0 && (
          <details className="mt-4 bg-white rounded-lg border overflow-hidden">
            <summary className="px-4 py-3 cursor-pointer hover:bg-gray-50 font-medium">📋 Event Log ({events.length})</summary>
      ...
      <footer className="mt-12 py-6 border-t bg-white">
        <div className="max-w-7xl mx-auto px-4 text-center text-sm text-gray-400">
          <p>Data: Apple App Store RSS feeds. Analysis: LLM semantic classification. Findings linked to source reviews.</p>
        </div>
      </footer>
    </div>
  );
